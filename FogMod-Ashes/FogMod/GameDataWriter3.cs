...
namespace FogMod
{
    public class GameDataWriter3
    {
        ...
        public void Write(RandomizerOptions opt, AnnotationData ann, Graph g, string gameDir, string outDir, Events events, EventConfig eventConfig)
        {
            ...
            editor.Spec.GameDir = $@"fogdist-ashes";
            editor.Spec.LayoutDir = $@"fogdist-ashes\Layouts";
            editor.Spec.NameDir = $@"fogdist-ashes\Names";
            ...
            {
                string path = @"fogdist-ashes\Base\Data0.bdt";
            ...
            foreach (string basePath in Directory.GetFiles($@"fogdist-ashes\Base", "*.msb.dcx"))
            {
            ...
            foreach (string basePath in Directory.GetFiles($@"fogdist-ashes\Base", "*.emevd.dcx"))
            {
                string path = basePath;
                string name = GameEditor.BaseName(path);
            ...
            foreach (string basePath in Directory.GetFiles($@"fogdist-ashes\Base", "*.talkesdbnd.dcx"))
            {
                string path = basePath;
                string name = GameEditor.BaseName(path);
                if (!ann.Specs.ContainsKey(name)) continue;
            ...
            {
                string basePath = $@"fogdist-ashes\Base\Data0.bdt";
                string altPath = $@"{gameDir}\Data0.bdt";
            ...
            foreach (KeyValuePair<string, Dictionary<string, ESD>> entry in esds)
            {
                string basePath = $@"fogdist-ashes\Base\{entry.Key}.talkesdbnd.dcx";
