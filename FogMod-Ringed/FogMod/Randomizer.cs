...
namespace FogMod
{
    public class Randomizer
    {
        public ItemReader.Result Randomize(RandomizerOptions opt, FromGame game, string gameDir, string outDir)
        {
            ...
            string file = game == FromGame.DS3 ? @"fogdist-ringed\fog.txt" : @"dist\fog.txt";
            ...
            {
                using (var f = File.OpenText(@"fogdist-ringed\locations.txt")) ann.Locations = deserializer.Deserialize<AnnotationData.FogLocations>(f);
                events = new Events(@"fogdist-ringed\Base\ds3-common.emedf.json");
            }
            ...
            if (game == FromGame.DS3)
            {
                ...
                using (var f = File.OpenText(@"fogdist-ringed\events.txt")) eventConfig = deserializer.Deserialize<EventConfig>(f);
