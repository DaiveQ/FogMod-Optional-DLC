/* 
 * Modifications Copyright (C) 2020 Karanveer B
*/
...
namespace FogMod
{
    public class GameDataWriter3
    {
        ...
        public void Write(RandomizerOptions opt, AnnotationData ann, Graph g, string gameDir, string outDir, Events events, EventConfig eventConfig)
        {
            ...
            editor.Spec.GameDir = $@"fogdist-ringed";
            editor.Spec.LayoutDir = $@"fogdist-ringed\Layouts";
            editor.Spec.NameDir = $@"fogdist-ringed\Names";
            ...
            {
                string path = @"fogdist-ringed\Base\Data0.bdt";
            ...
            {
                string path = @"fogdist-ringed\Base\menu_dlc2.msgbnd.dcx";
            ...
            foreach (string basePath in Directory.GetFiles($@"fogdist-ringed\Base", "*.msb.dcx"))
            {
            ...
            foreach (string basePath in Directory.GetFiles($@"fogdist-ringed\Base", "*.emevd.dcx"))
            {
                string path = basePath;
                string name = GameEditor.BaseName(path);
            ...
            foreach (string basePath in Directory.GetFiles($@"fogdist-ringed\Base", "*.talkesdbnd.dcx"))
            {
                string path = basePath;
                string name = GameEditor.BaseName(path);
                if (!ann.Specs.ContainsKey(name)) continue;
            ...
            {
                string basePath = $@"fogdist-ringed\Base\Data0.bdt";
                string altPath = $@"{gameDir}\Data0.bdt";
            ...
            {
                string basePath = $@"fogdist-ringed\Base\menu_dlc2.msgbnd.dcx";
                string altPath = $@"{gameDir}\msg\engus\menu_dlc2.msgbnd.dcx";
                if (File.Exists(altPath))
                {
                    basePath = altPath;
            ...
            foreach (KeyValuePair<string, Dictionary<string, ESD>> entry in esds)
            {
                string basePath = $@"fogdist-ringed\Base\{entry.Key}.talkesdbnd.dcx";
