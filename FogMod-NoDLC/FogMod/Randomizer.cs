...
namespace FogMod
{
    public class Randomizer
    {
        public ItemReader.Result Randomize(RandomizerOptions opt, FromGame game, string gameDir, string outDir)
        {
            ...
            string file = game == FromGame.DS3 ? @"fogdist-nodlc\fog.txt" : @"dist\fog.txt";
            ...
            {
                using (var f = File.OpenText(@"fogdist-nodlc\locations.txt")) ann.Locations = deserializer.Deserialize<AnnotationData.FogLocations>(f);
                events = new Events(@"fogdist-nodlc\Base\ds3-common.emedf.json");
            }
            ...
            if (game == FromGame.DS3)
            {
                ...
                using (var f = File.OpenText(@"fogdist-nodlc\events.txt")) eventConfig = deserializer.Deserialize<EventConfig>(f);
