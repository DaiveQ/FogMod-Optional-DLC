...
namespace FogMod
{
    public class GameDataWriter3
    {
        ...
        public void Write(RandomizerOptions opt, AnnotationData ann, Graph g, string gameDir, string outDir, Events events, EventConfig eventConfig)
        {
            ...
            editor.Spec.GameDir = $@"fogdist-nodlc";
            editor.Spec.LayoutDir = $@"fogdist-nodlc\Layouts";
            editor.Spec.NameDir = $@"fogdist-nodlc\Names";
            ...
            {
                string path = @"fogdist-nodlc\Base\Data0.bdt";
            ...
            foreach (string basePath in Directory.GetFiles($@"fogdist-nodlc\Base", "*.msb.dcx"))
            {
            ...
            foreach (string basePath in Directory.GetFiles($@"fogdist-nodlc\Base", "*.emevd.dcx"))
            {
                string path = basePath;
                string name = GameEditor.BaseName(path);
            ...
            foreach (string basePath in Directory.GetFiles($@"fogdist-nodlc\Base", "*.talkesdbnd.dcx"))
            {
                string path = basePath;
                string name = GameEditor.BaseName(path);
                if (!ann.Specs.ContainsKey(name)) continue;
            ...
            {
                string basePath = $@"fogdist-nodlc\Base\Data0.bdt";
                string altPath = $@"{gameDir}\Data0.bdt";
            ...
            foreach (KeyValuePair<string, Dictionary<string, ESD>> entry in esds)
            {
                string basePath = $@"fogdist-nodlc\Base\{entry.Key}.talkesdbnd.dcx";
